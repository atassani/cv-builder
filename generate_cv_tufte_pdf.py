...
Usage:
    python3 generate_cv_tufte_pdf.py cv_toni_tassani.json
    python3 generate_cv_tufte_pdf.py cv_toni_tassani.json --output my_resume.pdf
"""
...
def parse_args():
    ...
    parser.add_argument(
        "-o",
        "--output",
        help="Optional output PDF path. Defaults to <json_basename>_tufte.pdf",
    )
...
def derive_output_path(data, json_path, cli_output=None):
    ...
    if data.get("output_tufte_pdf"):
        return resolve_path(data["output_tufte_pdf"])
